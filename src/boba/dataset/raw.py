...
dataset is an ordered tuple of template+args specs (:class:`boba.dataset.columns.ColumnSpec`);
...
from boba.dataset.columns import ColumnSpec, expand_columns, listing_spans
from boba.dataset.costs import OUTCOME_MS, CostConfig, _build_cost_tables, _cost_fields_for_grid
from boba.io import DATA_DIR, list_blocks, load_block
from boba.dataset.session_data import SessionData, build_session_data
...
class DatasetRawConfig:
    ...
    ordered tuple of :class:`boba.dataset.columns.ColumnSpec` (template + args; the
    template catalogue lives in docs/raw_features.md / boba.dataset.columns.TEMPLATES).
    ...
    # cost-field names you want (the catalogue is `boba.dataset.raw._COST_FIELDS`). The default
    ...
    only_on_event: bool = True

    # Row filter — keep only grid rows whose latest ms (t−1ms, t] contains a
    # trade or a microprice change on ≥1 generated listing. "Move" = any
    # aggregated BBO event that changes the microprice (price- OR qty-driven;
    # identical-state updates don't count). Uses the at-or-before window, so a
    # kept row's features already reflect the event that kept it (unlike
    # only_on_event's floor convention, which marks the ms CONTAINING the
    # event). Supersedes only_on_event when True. Folded into config_str →
    # cache keys never collide across the flag.
    only_on_trade_or_move: bool = False

    # Performance (does NOT affect dataset output — just parallelism)
    n_workers: int = 18                  # thread pool size for parallel column / chunk work
...
def compute_bbo_event_emas(
    ...
    (families: see boba.dataset.columns.TEMPLATES).
...
def build_features_raw(
    ...
    t1 = time.perf_counter()
    if cfg.only_on_trade_or_move:
        # Keep tick t iff a trade or a microprice-changing BBO event falls in
        # (t−1ms, t] on any listing — the at-or-before window (ceil), matching
        # forward-fill visibility: the kept row already reflects the event.
        # Move events = aggregated BBO events with abs_log_ret > 0 (exact 0.0
        # when the microprice is unchanged). Supersedes only_on_event.
    ...
    elif cfg.only_on_event:
        event_mask = np.zeros(N_full, bool)
        for e in sorted(listings):
            bbo_ms = (data.listing_book_t[e] // 1_000_000).astype(np.int64)
            tr_ms  = (data.trade_ts[e] // 1_000_000).astype(np.int64) if len(data.trade_ts[e]) > 0 else np.empty(0, np.int64)
            for ms_arr in (bbo_ms, tr_ms):
                if len(ms_arr) == 0:
                    continue
                # Filter to within grid range, convert to grid-relative indices
                in_range = (ms_arr >= grid_start_ms) & (ms_arr <= grid_end_ms)
                if not in_range.any():
                    continue
                idx = ms_arr[in_range] - grid_start_ms
                event_mask[idx] = True
            log(f"{_t()}   {e} mask updated ({len(bbo_ms):,} bbo + {len(tr_ms):,} trade ms)")
    else:
