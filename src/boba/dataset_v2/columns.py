...
TEMPLATES: dict[str, Template] = {t.template: t for t in (
    ...
    # …and on a foreign trade clock (@{CLOCK}) — the listing's own trade flow summed over each
    # clock interval (own clock ⇒ one trade per interval ⇒ identical to the bare column).
    _t("{LISTING}_ema_buy_trade_qty_{N}t@{CLOCK}",      "ema_buy_trade_qty_t"),
    _t("{LISTING}_ema_sell_trade_qty_{N}t@{CLOCK}",     "ema_sell_trade_qty_t"),
    _t("{LISTING}_ema_buy_trade_value_{N}t@{CLOCK}",    "ema_buy_trade_value_t"),
    _t("{LISTING}_ema_sell_trade_value_{N}t@{CLOCK}",   "ema_sell_trade_value_t"),
    _t("{LISTING}_ema_trade_serial_cov_{N}t@{CLOCK}",   "ema_trade_serial_cov_t"),
    ...
    # Realized vol of LISTING's MID over a trailing window of N trade-clock events (tick-EMA of
    # squared 1ms mid log-returns; the clock = window/decay ruler, the mid path is full-resolution).
    # The optional @{CLOCK} clocks it off ANOTHER listing's trades, so a busy and a quiet venue's
    # event-vol line up on the same trade stream — e.g. byb_…_vol_100t@bin_… = byb's mid-move vol
    # over bin's last-100-trade windows. Bare form ⇒ own trade clock.
    _t("{LISTING}_vol_{N}t",                            "vol_t"),
    _t("{LISTING}_vol_{N}t@{CLOCK}",                     "vol_t"),
    # Book-event quantities tick-EMA'd over a (possibly foreign @{CLOCK}) TRADE clock — the same
    # shared-clock idea as vol, for the `_b`/microprice quantities. Two semantics:
    #   LEVEL (state): sample the level at each clock trade (forward-fill), then tick-EMA.
    #   FLOW  (signed flow): SUM the quantity over each clock interval (full-resolution), then tick-EMA.
    # `_sq` squares the sampled level / the per-event flow before the EMA (variance reconstruction).
    # LEVEL families:
    _t("{LISTING}_ema_book_imbalance_{N}t",                 "ema_book_imbalance_t"),
    _t("{LISTING}_ema_book_imbalance_{N}t@{CLOCK}",          "ema_book_imbalance_t"),
    _t("{LISTING}_ema_book_imbalance_sq_{N}t",              "ema_book_imbalance_sq_t"),
    _t("{LISTING}_ema_book_imbalance_sq_{N}t@{CLOCK}",       "ema_book_imbalance_sq_t"),
    _t("{LISTING}_ema_book_depth_{N}t",                    "ema_book_depth_t"),
    _t("{LISTING}_ema_book_depth_{N}t@{CLOCK}",             "ema_book_depth_t"),
    _t("{LISTING}_ema_book_depth_sq_{N}t",                 "ema_book_depth_sq_t"),
    _t("{LISTING}_ema_book_depth_sq_{N}t@{CLOCK}",          "ema_book_depth_sq_t"),
    _t("{LISTING}_ema_spread_wide_flag_{N}t",              "ema_spread_wide_flag_t"),
    _t("{LISTING}_ema_spread_wide_flag_{N}t@{CLOCK}",       "ema_spread_wide_flag_t"),
    _t("{LISTING}_ema_microprice_centered_{N}t",           "ema_microprice_centered_t"),
    _t("{LISTING}_ema_microprice_centered_{N}t@{CLOCK}",    "ema_microprice_centered_t"),
    _t("{LISTING}_ema_microprice_centered_sq_{N}t",        "ema_microprice_centered_sq_t"),
    _t("{LISTING}_ema_microprice_centered_sq_{N}t@{CLOCK}", "ema_microprice_centered_sq_t"),
    # FLOW families:
    _t("{LISTING}_ema_ofi_{N}t",                          "ema_ofi_t"),
    _t("{LISTING}_ema_ofi_{N}t@{CLOCK}",                   "ema_ofi_t"),
    _t("{LISTING}_ema_ofi_sq_{N}t",                       "ema_ofi_sq_t"),
    _t("{LISTING}_ema_ofi_sq_{N}t@{CLOCK}",                "ema_ofi_sq_t"),
    _t("{LISTING}_ema_abs_log_ret_{N}t",                  "ema_abs_log_ret_t"),
    _t("{LISTING}_ema_abs_log_ret_{N}t@{CLOCK}",           "ema_abs_log_ret_t"),
...
class ColumnUnit:
    ...
    n: Optional[int]
    clock: Optional[str] = None          # event-clock families: the listing whose events clock
                                         # this column (the optional @{CLOCK}); None ⇒ own listing
...
_CLOCK_PREFIX = "trades_"


def _clock_listing(clock: Optional[str]) -> Optional[str]:
    """The listing whose trades a CLOCK token names: ``'trades_<listing>'`` → ``'<listing>'``.
    ``None`` (bare column ⇒ own clock) passes through as ``None``; a malformed token returns
    ``None`` too (caught by validation). Only the trade clock exists today."""
    if clock is None:
        return None
    if clock.startswith(_CLOCK_PREFIX) and len(clock) > len(_CLOCK_PREFIX):
        return clock[len(_CLOCK_PREFIX):]
    return None
...
def _validate_binding(t: Template, b: dict[str, Any], listings: Optional[Sequence[str]], where: str) -> None:
    for p in t.placeholders:
        ...
        if p == "N":
            ...
            # an event clock names BOTH its event type and its listing: "trades_<listing>"
            # (only the trade clock exists today; the prefix leaves room for book_<listing> etc.).
            cl = _clock_listing(v)
            if cl is None:
                raise ValueError(f"{where}: CLOCK must be 'trades_<listing>', got {v!r}")
            if listings is not None and cl not in listings:
                raise ValueError(
                    f"{where}: CLOCK listing {cl!r} is not one of the listings {sorted(listings)}")
        else:
...
def _unit(t: Template, b: dict[str, Any]) -> ColumnUnit:
    ...
    return ColumnUnit(name=name, family=t.family, listing=b["LISTING"],
                      local_name=local, pair=None, n=b.get("N"), clock=b.get("CLOCK"))
    ...
    for si, spec in enumerate(specs):
        ...
        if t is None:
            raise ValueError(
                ...
                f"see boba.dataset_v2.columns.TEMPLATES / docs/raw_features.md")
        if not spec.args:
