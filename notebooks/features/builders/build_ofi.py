"""Generate notebooks/features/ofi.ipynb — the OFI (order-flow-imbalance) feature analysis.
...
- **React to every relevant event, read the freshest value.** Decay rides the trade clock, but each
  ...
  last-trade snapshot.
- **Records sharing a timestamp are ONE event.** A burst of byb book rows stamped at the exact same
  nanosecond is **one observation of the book**, not a sequence. We collapse them to the final
  top-of-book at that timestamp and compute **one** OFI increment from the previous distinct
  timestamp's book — never N−1 phantom intra-instant increments. (The decay clock likewise advances
...
top-of-book rows of byb's book (`prev` → `cur`):
...
Now the new piece: the **OFI increment stream**. Load byb's raw `front_levels`, collapse
same-timestamp rows to the final top-of-book (records sharing a nanosecond are ONE event), and form
one OFI increment `e` per consecutive distinct-timestamp pair by the §2 formula. That gives an event
stream `(ofi_rx, ofi_e)` — one increment per byb book change — that we decay on the trade clock and
read as `E/W`, *exactly* like `mv_rx, mv_r2` feeds `σ_ev`. The same `_flow_at` machine serves both.
""")

code(r"""
# --- the OFI flow on byb's own book: one increment e per distinct-timestamp book change ---
...
anchor; it never reads a stale last-trade state. Same-nanosecond byb book rows are one event (final
...
- [x] the data quirks handled (bad zero-price prints filtered; byb front_levels qty used; same-ns rows collapsed)
""")
...
out = Path("/Users/leith/source/new/boba/notebooks/features/ofi.ipynb")
