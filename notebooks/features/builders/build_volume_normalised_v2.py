"""Generate notebooks/features/volume_normalised_v2.ipynb — feature analysis for `volume_normalised`,
...
Feature:  volume_normalised(ex, span) = ema(volume) / yardstick
  - ema(volume): a SINGLE span-N KernelMeanEMA (E/W) of per-trade-event traded qty for venue `ex`
    ...
    MEAN traded size per trade-event (self-normalising E/W).
...
trade clock; SWEPT as a full single-span EMA family. INTENSITY/regime descriptor -> rate head, control."""
import json
...
out = Path(__file__).resolve().parents[1] / "volume_normalised_v2.ipynb"   # notebooks/features/volume_normalised_v2.ipynb, relative to this builder (portable across checkouts)
