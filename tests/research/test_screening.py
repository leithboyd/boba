...
from boba.features.base import Config, FeatureSpec, FrontLevels, ListingRaw, RawData, Trade
...
from boba.research.screening import (
    HeadConfig, RawEventStream, ScreeningContext,
...
# --------------------------------------------------------------------------------------------------
def _live_mid_spec():
    """A throwaway feature on the NEW contract: value() = each listing's live mid; vectorized = the causal
    mid at every `event_ts`. Exercises the driver, the read-at-anchor timing + event->anchor sampling,
    fan-out, and key agreement — must match exactly (no EMA)."""

    class LiveMid:
        def __init__(self, config, params):
            self.keys = tuple(_ex(l) for l in config.all_listings)
            self._key = {l: _ex(l) for l in config.all_listings}
            self._m: dict = {}
    ...
    def vec(raw, shared, config, params):
        return {_ex(l): _ffill(shared.listings[l].mid.rx, shared.listings[l].mid.value, shared.event_ts)
                for l in config.all_listings}

    return FeatureSpec("livemid_test", vec, lambda config, p: LiveMid(config, p),
                       lambda config, p: tuple(_ex(l) for l in config.all_listings))
...
def _synthetic_event_ctx():
    ...
    trade_ts = np.array([15, 35, 55, 75], np.int64)               # the shared decay clock (livemid ignores it)

    def fl(rx, mid):                                              # spread-0 book so (bid+ask)/2 == mid exactly
        return FrontLevels(rx, rx, mid, np.ones(len(rx)), mid, np.ones(len(rx)))
    nt = len(trade_ts)
    raw_data = RawData(listings={
        "aaa_x": ListingRaw(front_levels=fl(rx_a, mid_a),
                            trade=Trade(trade_ts, trade_ts, np.full(nt, 100.), np.ones(nt), np.ones(nt))),
        "bbb_x": ListingRaw(front_levels=fl(rx_b, mid_b),
                            trade=Trade(np.empty(0, np.int64), np.empty(0, np.int64), np.empty(0), np.empty(0), np.empty(0))),
    })
    config = Config("aaa_x", ("bbb_x",), coin, {"aaa_x": "front_levels", "bbb_x": "front_levels"}, yardstick_span=10)
    shared = build_shared_data(raw_data, config)

    rx = np.concatenate([rx_a, rx_b])                            # book events (kind 0) for the parity driver
    ...
    c = np.ones(len(rx)); d = c.copy()                           # dummy unit sizes
    order = np.lexsort((kind, rx))
    raw_events = RawEventStream(rx[order], kind[order], lid[order], t[order], a[order], b[order],
                               c[order], d[order], ("aaa_x", "bbb_x"))
    anchor_ts = np.array([25, 45, 65, 85, 95], np.int64)
    return ScreeningContext(
        block="syn", coin=coin, target="aaa_x", sources=("bbb_x",), horizon_ns=0, yardstick_span=10,
        mid_stream={"aaa_x": "front_levels", "bbb_x": "front_levels"}, merged_ts=shared.clock,
        anchor_ts=anchor_ts, sigma_at_anchor=np.empty(0), lam_at_anchor=np.empty(0),
        price_target=np.empty(0), rate_target=np.empty(0), base=[], vol_level=np.empty(0), rate_level=np.empty(0),
        vol_regime=np.empty(0), raw_events=raw_events, raw_data=raw_data, shared_data=shared, config=config)
...
def _gate_ctx(n=20000, seed=0):
    ...
    ctx = ScreeningContext(
        ...
        mid_stream={}, merged_ts=np.empty(0), anchor_ts=np.empty(0),
...
def test_echo_netted_ic_real_vs_echo():
    ...
    ctx = ScreeningContext(
        ...
        mid_stream={}, merged_ts=np.empty(0), anchor_ts=anchors,
...
# --------------------------------------------------------------------------------------------------
# build_context wiring — config.tick_size is populated for exactly the fused listings (real block)
# --------------------------------------------------------------------------------------------------
@pytest.mark.skipif(getattr(io, "DATA_DIR", None) is None, reason="needs DATA_DIR (build_context)")
def test_build_context_populates_tick_size_for_fused_listings():
    """build_context wires `config.tick_size = {fused listing -> io.tick_size}` — exactly the merged_levels
    listings, each a positive tick straight from the config loader. A fused listing with no configured tick
    fails fast inside io.tick_size (tested in test_io_merged_levels); here we pin the build_context wiring."""
    from boba.research.screening import build_context
    ctx = build_context(hours=1)
    fused = {l for l, s in ctx.config.mid_stream.items() if s == "merged_levels"}
    assert fused                                                  # the default head fuses at least one source
    assert set(ctx.config.tick_size) == fused                     # exactly the fused listings, no more/less
    for l in fused:
        assert ctx.config.tick_size[l] == io.tick_size(l) > 0.0   # wired straight from the config loader
