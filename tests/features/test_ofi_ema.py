...
NO shared code with production:
  ...
  - `test_streaming_matches_vectorized`  drive `LiveOFIEma` over a synthetic raw-event stream and assert
                                         it matches the (sampled) vectorized via `parity_check`.
...
def test_vectorized_matches_oracle():
    ...
    for n in (1, 10, 100):
        ...
        ref = _ofi_leg_oracle(book, trade_ts, shared.event_ts, n)
...
def test_span1_finite_where_inputs_exist():
    ...
    raw, shared, config = _inputs({"byb": book}, trade_ts)
    spec = base.get("ofi_ema")
    got = spec.vectorized(raw, shared, config, 1)["byb"]
    ref = _ofi_leg_oracle(book, trade_ts, shared.event_ts, 1)
    assert np.isfinite(got).sum() > 100                         # the span=1 leg is well-populated, not degenerate
    assert not np.isinf(got).any()                              # never a spurious inf at α=1
    # finite EXACTLY where the independent oracle is (a consistent NaN both builds agree on — α=1 is NaN
    # only at a bare trade-tick, where E/W decays to 0/0, never spuriously elsewhere)
    np.testing.assert_array_equal(np.isnan(got), np.isnan(ref))
    fin = np.isfinite(ref)
    np.testing.assert_allclose(got[fin], ref[fin], rtol=1e-9, atol=1e-9)
...
def test_streaming_matches_vectorized():
    ...
    rep = parity_check(ctx, base.get("ofi_ema"), [1, 10, 100], n_grid=len(anchor_ts), tol=1e-9)
...
def test_real_block_parity():
    ...
    rep = parity_check(ctx, base.get("ofi_ema"), [1, 100], tol=1e-6)
    assert rep.passed, str(rep)
